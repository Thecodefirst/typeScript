  ...
  <div class="app-container">
    <div class="filter-container">
      <el-input
        ...
        :placeholder="$t('departmentTable.searchDepartmentName')"
        style="width: 200px;"
        class="filter-item"
    ...
    >
      ...
      <el-table-column
        :label="$t('departmentList.create_date')"
      ...
      <!-- <el-table-column
        :label="$t('departmentList.create_userid')"
        class-name="status-col"
      >
        <template slot-scope="{row}">
      ...
      <el-table-column
        fixed="right"
        :label="$t('departmentList.operate')"
        align="center"
        width="200"
    ...
    >
      ...
      >
        <el-form-item
          ...
          prop="name"
        >
          <el-input v-model="tempDepartmentData.name" />
        </el-form-item>
      </el-form>
      <div
          slot="footer"
...
import { updateDepartment, deleteDepartment, getDepartmentList, addDepartment, defaultDepartmentListData } from '@/api/departmentManagement'
import { IDepartmentListData } from '@/api/types'
...
export default class extends Vue {
  private tableKey = 0
  private list: IDepartmentListData[] = []
  private total = 1
  private count = 10
  ...
  private tempDepartmentData = defaultDepartmentListData
  ...
  private resetTempDepartmentData() {
    this.tempDepartmentData = cloneDeep(defaultDepartmentListData)
  }

  private async handleCreate() {
    this.resetTempDepartmentData()
  ...
  private async createData() {
    (this.$refs.dataForm as Form).validate(async(valid) => {
      if (valid) {
        const { data } = await addDepartment(this.tempDepartmentData)
  ...
  private handleUpdate(row: any) {
    this.tempDepartmentData = Object.assign({}, row)
  ...
  private updateData() {
    (this.$refs.dataForm as Form).validate(async(valid) => {
      if (valid) {
        const { data } = await updateDepartment(this.tempDepartmentData)
        if (data.status === 'success') {
