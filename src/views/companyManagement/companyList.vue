<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        :label="$t('companyList.create_date')"
      ...
      <!-- <el-table-column
        :label="$t('companyList.create_userid')"
        class-name="status-col"
      >
        <template slot-scope="{row}">
    ...
    >
      ...
      >
        <el-form-item
          ...
          prop="name"
        >
          <el-input v-model="tempCompanyData.name" />
        </el-form-item>
      </el-form>
      <div
          slot="footer"
...
import { updateCompany, deleteCompany, getCompanyList, addCompany, defaultCompanyListData } from '@/api/companyManagement'
import { ICompanyListData } from '@/api/types'
...
export default class extends Vue {
  ...
  private tempCompanyData = defaultCompanyListData
  ...
  private resetTempCompanyData() {
    this.tempCompanyData = cloneDeep(defaultCompanyListData)
  }
  ...
  private async createData() {
    (this.$refs.dataForm as Form).validate(async(valid) => {
      if (valid) {
        const { data } = await addCompany(this.tempCompanyData)
  ...
  private updateData() {
    (this.$refs.dataForm as Form).validate(async(valid) => {
      if (valid) {
        const { data } = await updateCompany(this.tempCompanyData)
        if (data.status === 'success') {
  ...
  private async handleDelete(row: any, index: number) {
    const { data } = await deleteCompany({ id: row.id })
    if (data.status === 'success') {
      this.$notify({
        title: '成功',
        message: '删除成功',
